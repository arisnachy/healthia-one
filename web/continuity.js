...
(() => {
  ...
  function renderTimeline(){const root=$("#timelineRoot");if(!root||!snapshot)return;const events=snapshot.timeline||[],packs=snapshot.condition_packs||[],spanish=i18n?.locale==="es";root.innerHTML=`<div class="continuity-layout"><div class="condition-pack-grid">${packs.map(pack=>`<article class="condition-pack"><h3>${escapeHtml(spanish?pack.label:(pack.label_en||pack.label))}</h3><p>${(spanish?pack.signals:(pack.signals_en||pack.signals)).map(escapeHtml).join(" · ")}</p><ul>${(spanish?pack.questions:(pack.questions_en||pack.questions)).map(q=>`<li>${escapeHtml(q)}</li>`).join("")}</ul></article>`).join("")}</div><div class="timeline-list">${events.length?events.map(event=>`<article class="timeline-event"><div><h3>${escapeHtml(spanish?event.title:(event.title_en||event.title))}</h3><p>${escapeHtml(spanish?event.detail:(event.detail_en||event.detail))} · ${escapeHtml(event.source)}</p></div><small>${formatDate(event.occurred_at)}</small></article>`).join(""):`<article class="timeline-event"><div><h3>${text("No events yet","Sin eventos")}</h3><p>${text("Record a measurement or upload a document.","Registra una medición o carga un documento.")}</p></div></article>`}</div></div>`;}
