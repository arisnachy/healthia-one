...
from healthia_one.models import DeviceMetric, DeviceObservation, PatientState, RiskLevel
...
def _exercise_active(metadata: dict[str, Any]) -> bool:
    raw = metadata.get("exercise_session_active")
    if isinstance(raw, bool):
        return raw
    return str(raw or "").strip().lower() in {"1", "true", "yes", "active"}
...
def assess_device_context(
    ...
    *,
    ...
    """Interpret a device signal in context without overriding the safety layer.

    Guardian only consumes semantic context explicitly supplied by the authorized
    device bridge. Precise coordinates are intentionally ignored here. The output
    distinguishes observation, inference and hypothesis so association is never
    silently promoted to causation.
    """
    ...
    if repeated and materially_above_baseline:
        ...
        return GuardianAssessment(
            ...
            risk_level=RiskLevel.WATCH,
            summary=(
                f"A repeated heart-rate pattern was detected around the same time and {location_label} context."
            ),
            observed=observed,
