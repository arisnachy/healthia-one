...
## Current recording status — PASS

A continuous, unmocked judge-demo recording has already been captured successfully by the repository's gated Cloud workflow.
...
- live Cloud Run revision: `healthia-one-demo-00016-mct`
- synthetic data only
- duration gate: PASS
- zero browser console/page errors

The recorder reused the existing private Cloud Run deployment; it did not deploy another revision. The exact sanitized metadata is preserved in `hackathon/evidence/final_judge_demo_proof.json`.

**Remaining publication boundary:** upload the proven WebM unchanged to the final stable judge-facing video host and place that URL in `docs/DEVPOST_SUBMISSION.md`.
...
Switch to Google Cloud Console or show the live `.run.app` URL plus `/api/readiness` evidence.

**Show:**

- Cloud Run service `healthia-one-demo` or its `.run.app` URL;
- current ready revision;
- project `healthia-6088a` / region `us-central1`;
- live readiness showing Gemini 3.5 Flash, ADK, Firestore and GCS;
...
**PUBLIC/JUDGE VIDEO URL: TODO — publish the proven WebM from artifact `9024139098`.**

Do not mark the project `100/100` or `SUBMISSION_LOCKED` until the published URL is linked in `docs/DEVPOST_SUBMISSION.md`, the exact final submission head passes CI + JUDGE Ω, and PR #29 is merged/locked.
