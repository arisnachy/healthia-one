...
- Preferred human configuration gate is ONE protected GitHub Actions repository secret named `HEALTHIA_FIREBASE_ANDROID_CONFIG_JSON`, containing the official Firebase `google-services.json`. The workflow requires exactly one Android client for package `com.healthia.one.bridge`, project id `healthia-6088a`, masks extracted values and never uploads the source JSON. The four individual `HEALTHIA_FIREBASE_*` Actions secrets remain an optional fallback.
...
- Google Constellation provider preflight, Document AI/GCS preflight, FCM provider preflight, Firebase-config readiness and Android compile/readiness all completed without claiming a false LIVE or FCM-ready result.

NEXT HUMAN GATES
- Firebase Android config: obtain the official `google-services.json` for Firebase Android package `com.healthia.one.bridge` in project `healthia-6088a` (register that Android app first if it is absent) and save the full file as GitHub Actions secret `HEALTHIA_FIREBASE_ANDROID_CONFIG_JSON`. Do not paste it into chat, logs, issues, PRs or artifacts.
